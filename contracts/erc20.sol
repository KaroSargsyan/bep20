...
contract MyToken is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x45f3cd40c2a25dDB9742F65Bc35B8cD55D14B27B;
    ...
    function isWhitelisted(address addr) public view returns (bool isWhiteListed) {
        return whitelist[addr] == addr;
    }

    function addAddresstoBlacklist(address addr) onlyOwner public {
        require(!isBlacklisted(addr), "Already blacklisted");
        blacklist[addr]=addr;
    }

    function isBlacklisted(address addr) public view returns (bool isBlackListed) {
        return blacklist[addr] == addr;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount-tFee-tLiquidity-tDevelopment;
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount-rFee-rLiquidity-rDevelopment;
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        uint256 currentRate =  _getRate();
        uint256 rDevelopment = tDevelopment*currentRate;
        _rOwned[_developmentWalletAddress] = _rOwned[_developmentWalletAddress]+rDevelopment;
        if(_isExcluded[_developmentWalletAddress])
            _tOwned[_developmentWalletAddress] = _tOwned[_developmentWalletAddress]+tDevelopment;
    }
    ...
    function calculateDevelopmentFee(uint256 _amount) private view returns (uint256) {
        return _amount * _developmentFee/ 10**3;
    }
